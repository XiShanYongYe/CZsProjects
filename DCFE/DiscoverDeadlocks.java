...
public class DiscoverDeadlocks {
	...
	private List<PetriNetMy> petriNets = new ArrayList<>();
	...
	public List<PetriNetMy> getPetriNets() {
		return petriNets;
	}
	...
	public void mainBody() {
		...
		for (String event : allevents) {
			...
			if (!event.equals("te") && findDeadlock(event)) {
				...
				PetriNetMy petriNet = new PetriNetMy();
				...
				AerfaMiner aerfaMiner = new AerfaMiner(Tf, allevents, relations, casuals);
				...
				for (String string : or_spiltList) {
					...
					for (String target : targetsList) {
						...
						if (type == Constant.TRANSITVE_CLOSURE_OF_CAUSAL_RELATION) {
							PetriArc arc = new PetriArc(new PetriTransition(target), pi);
							F.add(arc);
						}
					}
	...
	private void anotherToGetTs(List<String> tf) {
		...
		for (String tString : tf) {
			...
			for (Relation relation : transitions) {
				...
				if (relation.getLeftTransition().equals(tString) && !relation.getRightTransition().equals(tString)) {
					hasNext = true;
					break;
				}
						...
						for (String target : targetsList) {
							...
							for (Relation relation : transitions) {
								...
								if (relation.getLeftTransition().equals(target)
										&& matchList.contains(relation.getRightTransition())) {
